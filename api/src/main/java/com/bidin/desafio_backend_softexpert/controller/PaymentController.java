    ...
    public ResponseEntity<List<PaymentResponseDTO>> generateLinks(@Valid @RequestBody PaymentRequestDTO request) {
        try {
            ...
            request.getShares().forEach((name, value) -> {
                PaymentResponseDTO user = new PaymentResponseDTO(name, value);
                if (!name.equals(request.getOwnerName())) {
                    var paymentInfo = paymentService.generateLink("Sua fatia", new BigDecimal(value), request.getOwnerPixKey());
